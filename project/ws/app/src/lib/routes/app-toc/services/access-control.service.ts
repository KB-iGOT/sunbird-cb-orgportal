...
import { ISearchContent } from '../models/search'
import { AUTHORING_CONTENT_BASE } from '../models/api-end-points'
...
export class AccessControlService {
  ...
  getIcon(content: NsContent.IContentMeta | ISearchContent): string {
    if (content.mimeType === NsContent.MIME_TYPE.collection) {
      if (this.getCategory(content) === 'Knowledge Board') {
        return NsContent.ICON_TYPE.kBoard
      }
      if (this.getCategory(content) === 'Learning Path') {
        return NsContent.ICON_TYPE.program
      }
      if (this.getCategory(content) === 'Course') {
        return NsContent.ICON_TYPE.course
      }
      return NsContent.ICON_TYPE.learningModule
    }
    if (content.mimeType === NsContent.MIME_TYPE.html) {
      if (content.resourceType === 'Certification') {
        return NsContent.ICON_TYPE.certificate
      }
      if (content.isExternal) {
        return NsContent.ICON_TYPE.externalContent
      }
      return NsContent.ICON_TYPE.internalContent
    }
    if (content.mimeType === NsContent.MIME_TYPE.pdf) {
      if (!content.artifactUrl) {
        return NsContent.ICON_TYPE.emptyFile
      }
      return NsContent.ICON_TYPE.pdf
    }
    if (content.mimeType === NsContent.MIME_TYPE.youtube) {
      return NsContent.ICON_TYPE.youtube
    }
    if (content.mimeType === NsContent.MIME_TYPE.quiz) {
      if (this.getCategoryType(content) === 'Assessment') {
        return NsContent.ICON_TYPE.assessment
      }
      return NsContent.ICON_TYPE.quiz
    }
    if (content.mimeType === NsContent.MIME_TYPE.dragDrop) {
      return NsContent.ICON_TYPE.dragNDrop
    }
    if (content.mimeType === NsContent.MIME_TYPE.htmlPicker) {
      return NsContent.ICON_TYPE.htmlPicker
    }
    if (content.mimeType === NsContent.MIME_TYPE.webModule) {
      return NsContent.ICON_TYPE.internalContent
    }
    if (content.mimeType === NsContent.MIME_TYPE.handson) {
      return NsContent.ICON_TYPE.handsOn
    }
    if (content.mimeType === NsContent.MIME_TYPE.iap) {
      return NsContent.ICON_TYPE.iap
    }
    if (content.mimeType === NsContent.MIME_TYPE.mp3) {
      return NsContent.ICON_TYPE.audio
    }
    if (content.mimeType === NsContent.MIME_TYPE.mp4) {
      return NsContent.ICON_TYPE.video
    }
    return NsContent.ICON_TYPE.default
  }
