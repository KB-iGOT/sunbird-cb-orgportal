...
export class AppTocBannerComponent implements OnInit, OnChanges, OnDestroy {
  ...
  getUsersToShare(queryStr: string) {
    ...
    this.userAutoComplete.searchUser(queryStr, this.rootOrgId).subscribe((data: any) => {
  ...
  public checkWithdrawn(batch: any) {
    if (
      ...
      if (this.selectedBatch.batchId === this.batchData.workFlow.wfItem.applicationId
        && (this.batchData.workFlow.wfItem.currentStatus === this.WFBlendedProgramStatus.WITHDRAWN ||
          this.batchData.workFlow.wfItem.currentStatus === this.WFBlendedProgramStatus.WITHDRAWN)) { // NOSONAR
        return true
      }
  ...
  private getLearningUrls() {
    if (this.content) {
      ...
      const firstPlayableContent: any = this.contentSvc.getFirstChildInHierarchy(this.content)
